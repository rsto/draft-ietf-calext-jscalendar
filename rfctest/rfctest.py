...
from .jsical import JDiff, JObject, JPath, VObject, ParseError
...
class Test:
    ...
    class Ical2JscalResult(Result):
        ...
        json_diff: JDiff = None
        """Diffed JSON response"""

        def is_valid(self) -> bool:
            return self.json_diff.empty()

    class Jscal2IcalResult(Result):
        ical_response: VObject = None
    ...
    def __init__(self, name: str, icaltext: str, jcaltext: str):
        ...
        self.vobject = VObject.parse(self.icaltext).to_vcalendar()
    ...
    def run(self, backend: Backend):
        try:
            ...
            self.i2jresult.json_response = JDiff.normalize_json(
        ...
        try:
            ...
            self.j2iresult.ical_response = VObject.parse(
        ...
        if not test.i2jresult.json_diff.empty():
            ...
            if jdiff.unequal:
                self._print_json_diff(
                    ...
                    (a, b),
                    (
                        [unq[0] for unq in jdiff.unequal],
                        [unq[1] for unq in jdiff.unequal],
                    ),
                    ("Expected", "Response"),
                )
            if jdiff.b_only:
